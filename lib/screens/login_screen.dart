...
class _LoginScreenState extends State<LoginScreen> {
  // Khởi tạo các bộ điều khiển dữ liệu cho ô nhập văn bản[cite: 2]
  final TextEditingController _emailController = TextEditingController();
  final TextEditingController _passwordController = TextEditingController();
  bool _isLoading = false; // Trạng thái hiển thị vòng xoay tải dữ liệu[cite: 2]
  bool _isRememberMe = false; // Trạng thái tích chọn ghi nhớ đăng nhập[cite: 2]
  List<Map<String, String>> _savedAccounts = []; // Danh sách chứa các tài khoản đã lưu cục bộ[cite: 2]

  // Hàm khởi tạo trạng thái mặc định của Widget[cite: 2]
  @override
  void initState() {
    super.initState();
    _loadSavedAccounts(); // Tự động quét và lấy danh sách tài khoản đã lưu ngay khi mở màn hình[cite: 2]
  }

  // Hàm giải phóng bộ nhớ khi hủy Widget[cite: 2]
  @override
  void dispose() {
    _emailController.dispose(); // Giải phóng bộ nhớ ô nhập email[cite: 2]
    _passwordController.dispose(); // Giải phóng bộ nhớ ô nhập mật khẩu[cite: 2]
    super.dispose();
  }

  // HÀM ĐỌC DỮ LIỆU CỤC BỘ: Lấy danh sách chuỗi JSON tài khoản đã lưu dưới máy và chuyển thành List[cite: 2]
  ...
  // HÀM GHI DỮ LIỆU CỤC BỘ: Mã hóa thông tin và lưu tài khoản xuống máy khi tick chọn Remember Me[cite: 2]
  Future<void> _saveAccountLocally(String email, String password) async {
    ...
    // Xóa tài khoản cũ nếu trùng email để tránh ghi đè lặp lại[cite: 2]
    accounts.removeWhere((element) => element['email'] == email);

    // Thêm thông tin tài khoản mới vào danh sách[cite: 2]
    accounts.add({
      'email': email,
      'password': password,
    ...
    // Chuyển List thành danh sách chuỗi JSON và lưu lại vào SharedPreferences[cite: 2]
  ...
  // HÀM XỬ LÝ ĐĂNG NHẬP: Gọi Firebase xác thực (hỗ trợ cả nhập thủ công và bấm đăng nhập nhanh)[cite: 2]
  void loginUser({String? customEmail, String? customPassword}) async {
    setState(() {
      _isLoading = true; // Bật hiệu ứng xoay tải dữ liệu[cite: 2]
    });

    // Xác định nguồn dữ liệu đầu vào (từ danh sách nhanh hoặc từ ô nhập)[cite: 2]
    ...
    // Nếu đăng nhập thành công và có tích chọn Remember Me thì tiến hành lưu lại[cite: 2]
    if (res == "Thành công" && _isRememberMe && customEmail == null) {
      await _saveAccountLocally(targetEmail, targetPassword);
    }

    setState(() {
      _isLoading = false; // Tắt hiệu ứng xoay tải dữ liệu[cite: 2]
    });
    ...
    if (res == "Thành công") {
      ...
      // Điều hướng sang màn hình HomeScreen và hủy bỏ màn hình Login hiện tại[cite: 2]
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(builder: (context) => const HomeScreen()),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(res)), // Hiển thị thông báo lỗi chi tiết[cite: 2]
      );
    }
  }
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                // VÙNG HIỂN THỊ DANH SÁCH TÀI KHOẢN ĐĂNG NHẬP NHANH (Nếu có)[cite: 2]
                ...
                // Các ô nhập dữ liệu thủ công[cite: 2]
                TextFieldInput(
                ...
                // Nút chuyển sang màn hình Đăng ký[cite: 2]
                Row(
                  ...
                  children: [
                    ...
                    TextButton(
                      onPressed: () {
                        // Thêm màn hình Đăng ký vào Stack điều hướng[cite: 2]
